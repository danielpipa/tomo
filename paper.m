...
tic;
layersX_hat2 = Cmtx*Hmtx'*pcg(A,WFSX,1e-6,1000);
layersY_hat2 = Cmtx*Hmtx'*pcg(A,WFSY,1e-6,1000);
Time_Matrix = toc;
fprintf('Elapsed Time = %f\n',Time_Matrix);
...
Target_hat_vis(subap_index) = TargetX_hat3;
figure;
...
figure;
imagesc(real(reshape(layersX_hat3(1:sizes(1)^2),sizes(1),[])))
title('layer 1 rec');
...
figure;
imagesc(real(reshape(layersX_hat3(sizes(1)^2+1:sizes(1)^2+sizes(2)^2),sizes(2),[])))
title('layer 2 rec');
...
figure;
imagesc(reshape(rec_target_hat3(1:(sizes(1)+1)^2),sizes(1)+1,[]))
title('Target WF rec');
...
figure;
imagesc(reshape(rec_layers_hat3(1:(sizes(1)+1)^2),sizes(1)+1,[]))
title('layer 1 WF rec');

%%

file_name = sprintf("paper_%.2d",size_i);
save(file_name);