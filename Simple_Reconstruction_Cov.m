...
% simulate WFS
%**************************************************
[WFS] = HHmex(layersX,x_shift,y_shift,sizes);
...
AA = @(x) HHmex(Cparmex(HHTmex(x,x_shift,y_shift,sizes),sizes,CovKernel),x_shift,y_shift,sizes) + sigma_2*x;
bb = WFS;
tic;
temp = pcg(AA,bb,1e-6,1000);
layersX_hat5 = Cparmex(HHTmex(temp,x_shift,y_shift,sizes),sizes,CovKernel);
Time_Fil2 = toc;
...
%**************************************************
%C full Covariance
Target_hat2 = HH_projection(layersX_hat2,subap_index,sizes,alt_p,WFS_target);
Target = HH_projection(layersX,subap_index,sizes,alt_p,WFS_target);
Error_Target_Full2 = mse(Target-Target_hat2)
Error_layers_Full2 = mse(layersX-layersX_hat2)

%C filter Covariance
Target_hat = HH_projection(layersX_hat,subap_index,sizes,alt_p,WFS_target);
Target = HH_projection(layersX,subap_index,sizes,alt_p,WFS_target);
Error_Target_Filter1 = mse(Target-Target_hat)
Error_layers_Filter1 = mse(layersX-layersX_hat)

%All full
Target_hat3 = HH_projection(layersX_hat3,subap_index,sizes,alt_p,WFS_target);
Target = HH_projection(layersX,subap_index,sizes,alt_p,WFS_target);
Error_Target_Full = mse(Target-Target_hat3)
Error_layers_Full = mse(layersX-layersX_hat3)

%All full without inverting C
Target_hat4 = HH_projection(layersX_hat4,subap_index,sizes,alt_p,WFS_target);
Target = HH_projection(layersX,subap_index,sizes,alt_p,WFS_target);
Error_Target_NIFull = mse(Target-Target_hat4)
Error_layers_NIFull = mse(layersX-layersX_hat4)

%Filter without inverting C
Target_hat5 = HH_projection(layersX_hat5,subap_index,sizes,alt_p,WFS_target);
Target = HH_projection(layersX,subap_index,sizes,alt_p,WFS_target);
